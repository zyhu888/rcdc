...
candidate value of `K`, save one result per task, and select the final model
using WAIC.

The template below follows an array-job workflow: one array task selects one
dataset or network, and each task evaluates candidate `K` values in parallel on
the allocated cores.
...
After all fixed-`K` runs finish, read the saved `.rds` files, compare WAIC, and
use the clustering from the run with the smallest WAIC.
