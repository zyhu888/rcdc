...
`rcdc()` evaluates candidate values of `K` serially. For larger analyses,
a more efficient workflow is to run `rcdc_MCMC()` separately for each
candidate value of `K`, save one result per candidate model, and select the
final model using WAIC.

The template below mirrors the small runnable example in `?rcdc`, but
